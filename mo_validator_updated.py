...
def validate_input(value, allowed_range):
    ...
    # Case 1: "X chars" or "min..max chars" for string length validation
    if "chars" in allowed_range:
        try:
            # Handle "min..max chars"
            if ".." in allowed_range:
                parts = allowed_range.split("..", 1)
                min_len = int(parts[0])
                max_len = int(parts[1].split()[0])
                if min_len <= len(str(value)) <= max_len:
                    return (True, None)
                else:
                    return (False, f"The value must be between {min_len} and {max_len} characters long.")
            # Handle "X chars"
            else:
